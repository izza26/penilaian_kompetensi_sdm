...
class AdminPenilaianController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($cari) {
            ...
            $result = DB::select($sql, ["%$cari%", "%$cari%", "%$cari%"]);
        } else {
            $sql .= " ORDER BY p.pegawai_nama ASC, uk.kode_unit ASC";
    ...
    public function form($pegawai_id, $kode_unit)
    {
        ...
        // KITA HAPUS JOIN KE rubrik_skor, GANTI MENGGUNAKAN kriteria_kompetens DARI aktivitas_kompeten
        ...
        // Cek apakah sudah pernah dinilai sebelumnya (Untuk Edit Mode)
    ...
    public function show($id)
    {
        $data = DB::table('pegawai as p')
            ->leftJoin('penilaian_header as ph', 'p.penilaian_id', '=', 'ph.penilaian_id')
            ->leftJoin('unit_kompetensi as uk', 'ph.kode_unit', '=', 'uk.kode_unit')
            ->leftJoin('peserta_penilaian as pp', 'p.pegawai_id', '=', 'pp.peserta_id')
            ->leftJoin('rekap_aktivitas_36 as ra', 'pp.rekap_aktivitas_id', '=', 'ra.rekap_aktivitas_id')
            ->where('p.pegawai_id', $id)
            ->select('p.pegawai_nama', 'p.jabatan', 'p.unit_kerja', 'uk.kode_unit', 'uk.judul_unit', 'ph.waktu_submit', 'ph.catatan_umum', 'ph.status as status_penilaian', 'ra.skor_akhir_360', 'ra.status_kompeten')
            ->first();

        if (!$data) return redirect()->route('admin.penilaian.index')->with('error', 'Data penilaian pegawai tidak ditemukan!');

        // Subquery aman untuk nama elemen
        ...
        $list_skor = DB::table('pegawai as p')
            ->join('penilaian_header as ph', 'p.penilaian_id', '=', 'ph.penilaian_id')
            ->join('penilaian_detail as pd', 'ph.detail_penilaian_i', '=', 'pd.detail_penilaian_i')
